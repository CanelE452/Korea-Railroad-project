...
def _regression_loss(prediction, target, reduction, loss_name, smooth_l1_beta):
    if loss_name.lower() in ("smooth_l1", "smoothl1", "huber"):
    ...
    raise ValueError("Unsupported regression loss: {}".format(loss_name))
...
class GeometrySMOKELossComputation():
    ...
    def __init__(self, smoke_coder, cls_loss, loss_weight, max_objs, regression_loss_name="L1", smooth_l1_beta=1.0):
        self.smoke_coder = smoke_coder
        self.cls_loss = cls_loss
        self.loss_weight = loss_weight
        self.max_objs = max_objs
    ...
    def __call__(self, predictions, targets):
        ...
        reg_loss = (
            ...
            + _regression_loss(
                pred_off_u * scalar_mask,
                p_offsets[:, :, 0] * scalar_mask,
                "sum",
                self.regression_loss_name,
                self.smooth_l1_beta,
            )
            + _regression_loss(
                pred_log_dv_delta * scalar_mask,
                (log_dv_gt - log_dv_ref) * scalar_mask,
                "sum",
                self.regression_loss_name,
                self.smooth_l1_beta,
            )
...
def make_smoke_loss_evaluator(cfg):
    ...
    if cfg.MODEL.SMOKE_HEAD.MODE == "geometry":
        loss_evaluator = GeometrySMOKELossComputation(
            ...
            regression_loss_name=cfg.MODEL.SMOKE_HEAD.REGRESSION_LOSS,
            smooth_l1_beta=cfg.MODEL.SMOKE_HEAD.SMOOTH_L1_BETA,
        )
