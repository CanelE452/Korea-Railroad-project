"""v8 vs v8ablationA vs v8ablationE - 3 model filter comparison on noapril."""
import csv
import os
import cv2
import numpy as np
...
MODELS = [
    ...
    ("ablE_rel", "data/pallet/eval_results/v8ablatione_noapril"),
]

FILTERS = ["A", "B", "C", "BC"]
OUT_BASE = "data/pallet/eval_results/v8_A_E_compare"
...
def get_score_label(filter_name, row):
    ...
    if filter_name == "A":
        ...
        return f"sp={row.get('span', '?')} sC={row.get('sC', '?')}"
    return ""


def make_row_image(images, labels, target_h=360):
    n = len(images)
    ...
    canvas = np.zeros((bar_h + cell_h, total_w, 3), dtype=np.uint8)

    for i in range(n):
        x_off = i * (cell_w + sep_w)
        cv2.putText(canvas, labels[i], (x_off + 4, bar_h - 10),
