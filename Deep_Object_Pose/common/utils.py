...
class CleanVisiiDopeLoader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        transform = A.Compose(
            [
                ...
                A.RandomBrightnessContrast(
                    brightness_limit=0.35, contrast_limit=0.2, p=1
                ),
                A.HueSaturationValue(
                    hue_shift_limit=10, sat_shift_limit=20, val_shift_limit=30, p=0.5
                ),
        ...
        # Per-keypoint visibility (geometry-derived self-occlusion)
    ...
    def _compute_visibility(self, data_json, img_shape, output_size):
        """Compute per-keypoint geometry-derived visibility.

        3 levels: visible(1.0), self-occluded(0.5), out-of-frame(0.0).
        Uses pose to determine which cuboid faces are front-facing.
        """
        H, W = img_shape[:2]
        ...
        if cuboid_3d is not None and len(cuboid_3d) >= 8 and pose_transform is not None:
            try:
                ...
                for fi, face in enumerate(self._CUBOID_FACES):
                    ...
                    # View direction: from face center toward camera (0,0,0)
                    face_center = pts_cam[face].mean(axis=0)
                    view_dir = -face_center  # camera at origin
                    face_visible[fi] = np.dot(normal, view_dir) > 0
            except Exception:
                pass  # fallback: all visible

        # Assign visibility per corner
        for i in range(8):
            ...
            if x < 0 or y < 0 or x >= W or y >= H:
                vis[i] = 0.0
                continue
            # At least one adjacent face is front-facing?
            any_visible = any(face_visible[fi] for fi in self._CORNER_FACES[i])
            vis[i] = 1.0 if any_visible else 0.5

        # Centroid: simple in-frame check (no self-occlusion concept)
