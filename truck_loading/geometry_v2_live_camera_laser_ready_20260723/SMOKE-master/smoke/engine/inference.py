...
def compute_on_dataset(model, data_loader, device, timer=None, use_fp16=False):
    model.eval()
    results_dict = {}
...
def inference(
    ...
    total_timer.tic()
    predictions = compute_on_dataset(model, data_loader, device, inference_timer, use_fp16=use_fp16)
    comm.synchronize()
