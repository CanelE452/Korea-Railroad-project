...
class DLA(nn.Module):
    def __init__(self,
        ...
        use_dcn = not cfg.MODEL.BACKBONE.CONV_BODY.endswith("-NODCN")
...
class IDAUp(nn.Module):
    def __init__(self,
        ...
        conv_func = DeformConv if use_dcn else ConvNormReLU

        for i in range(1, len(in_channels)):
            in_channel = in_channels[i]
            f = int(up_f[i])
            proj = conv_func(in_channel, out_channel, norm_func)
            node = conv_func(out_channel, out_channel, norm_func)
